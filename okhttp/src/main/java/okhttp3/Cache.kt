...
class Cache internal constructor(
  ...
  companion object {
    ...
    fun varyMatches(
      ...
      for (field in cachedResponse.headers().varyFields()) {
        if (cachedRequest.values(field) != newRequest.headers(field)) return false
      }
      return true
    }
    ...
    private fun Headers.varyFields(): Set<String> {
      ...
      for (i in 0 until size()) {
        ...
        for (varyField in value.split(',')) {
          (result as MutableSet<String>).add(varyField.trim())
        }
      }
      return result
    }
